...
from src.tests.data.test_data import RandomDeviceGenerator
...
class DeviceControllerTests(unittest.TestCase):
    ...
    def test_save_load_devices(self):
        ...
        from os import path
        self.assertFalse(path.exists(TEST_SAVE_PATH), "Save devices not found")

        # disconnect all devices and check size
        self.controller.remove_all()
        _connected_devices = self.controller.get_devices()
        self.assertNotEqual(0, len(_connected_devices), "Wrong number of connected devices")
